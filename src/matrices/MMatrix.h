...
typedef MatPorCajas<m_double> m_matrix;
...
class MMatrix: public m_matrix
  ...
  public:
    ...
    friend MMatrix operator*(const MMatrix &mM, const m_double &m)
      { return m*mM; }
    friend MMatrix a_cajas(const MMatrix &muestra,const m_double &a);
    friend MMatrix a_cajas(const std::vector<size_t> dim_n_rows,const std::vector<size_t> dim_n_columns,const m_double &a);
  };

...
class Polinomio : public Funcion, public mapPol
  ...
  public:
    Polinomio(const double &d= 0.0): Funcion(), mapPol() 
      { init(d); }
    Polinomio(const NmbVars &vars): Funcion(vars), mapPol() 
      { init(0.0); }      
    Polinomio(const char *vars): Funcion(vars), mapPol() 
      { init(0.0); }
    Polinomio(const char var): Funcion(var), mapPol() 
      { init(0.0); }
    Polinomio(const Polinomio &otro): Funcion(otro), mapPol(otro) {}
    ...
    friend bool operator ==(const Polinomio &p1,const Polinomio &p2)
      {
        if ((const Funcion &) p1 != (const Funcion &) p2) return 0;

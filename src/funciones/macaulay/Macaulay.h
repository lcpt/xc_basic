...
#include "../polinomios/funcion.h"

typedef vectorZ<double> vZ_double;

class Macaulay : public Funcion
  {
  protected:
    int n;
    double a;
    double coef;
  public:
    Macaulay(void): Funcion()
      { n= 0; a= 0.0; coef= 1.0;}
    Macaulay(const NmbVars &vars, const int n= 0, const double a= 0.0): Funcion(vars)
      { this->n= n; this->a= a; coef= 1; }
    Macaulay(const char *vars, const int n= 0, const double a= 0.0): Funcion(vars)
      { this->n= n; this->a= a; coef= 1; }
    Macaulay(const Macaulay &otro): Funcion(otro)
